...
<xsl:value-of select="floor( (140-$featureheight) div 28) "/>   -->
    ...
    <xsl:value-of select="system-property('first.page.weapons')"/>
  </xsl:template>
  ...
  <xsl:template match="weapons">
    ...
    <xsl:if test="position() &gt;= $first_weapon and $complex_melee = 'true'">
  ...
  <xsl:template match="melee">
    ...
    <fo:block keep-with-previous.within-page="always" keep-together.within-column="always">
      <xsl:choose>
        <xsl:when test="$complex_melee = 'true'">
          ...
          <fo:table table-layout="fixed">
            ...
            <fo:table-body>
              <fo:table-row keep-with-next.within-page="always">
                ...
                <fo:table-cell>
                  <xsl:call-template name="attrib"><xsl:with-param name="attribute" select="'weapon.title'"/></xsl:call-template>
                  <fo:block font-size="8pt" font-weight="bold" space-before="1pt">To Hit: <xsl:value-of select="w1_h1_p/to_hit"/></fo:block>
                </fo:table-cell>
                <fo:table-cell>
                  <xsl:call-template name="attrib"><xsl:with-param name="attribute" select="'weapon.title'"/></xsl:call-template>
                  <fo:block font-size="8pt" font-weight="bold" space-before="1pt">Damage: <xsl:value-of select="w1_h1_p/damage"/></fo:block>
                </fo:table-cell>
              </fo:table-row>
            </fo:table-body>
          </fo:table>
